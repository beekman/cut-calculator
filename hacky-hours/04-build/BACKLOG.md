...
### Pattern Repeat
- [ ] Add `RepeatDistance` and `RepeatAxis` fields to `StockPiece` model (`internal/model/types.go`)
...
### Join Groups
- [ ] Add `JoinGroup` field to `RequiredPiece` model (`internal/model/types.go`)
